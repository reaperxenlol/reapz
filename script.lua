    ...
    local success, err = pcall(function()
        ...
        pcall(function()
            ...
            elseif UserInputService.KeyboardEnabled then
                device = "PC"
                platform = "Desktop"
                inputType = UserInputService.GamepadEnabled and "Keyboard + Controller" or "Keyboard + Mouse"
            end
...
local function CreateTab(name, icon)
    ...
    function Tab:AddSection(title)
        ...
        function Section:AddButton(config)
            ...
            btnFrame.BackgroundTransparency = GUI_TRANSPARENCY
    ...
    self.Connection = RunService.PreSimulation:Connect(function()
        ...
        elseif mode == "Rage" then
            if Ball:GetAttribute("target") == Player.Name then
                ...
                elseif not Parried and Distance / Speed <= parryDistance then
                    VirtualInputManager:SendMouseButtonEvent(0, 0, 0, true, game, 0)
                    Parried = true
                    Cooldown = tick()
                end
